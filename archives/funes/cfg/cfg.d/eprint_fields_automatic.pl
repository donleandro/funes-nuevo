...
if( $type eq "conference_item" && !$eprint->is_set( "ispublished" ) )
	{
		$eprint->set_value( "ispublished", "unpub" );
		# conferences are usually unpublished.
	}

if( $type eq "monograph" && !$eprint->is_set( "ispublished" ) )
	{
		$eprint->set_value( "ispublished", "unpub" );
		# monographs are usually unpublished.
	}
