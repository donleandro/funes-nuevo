...
sub send_mail_via_smtp
{
	...
	#my $smtp = new Net::SMTP('smtp.uniandes.edu.co'); #,
				        #Hello   =>      'smtp.uniandes.edu.co',
				        #Port    =>      25, 
				        #User    =>      '',
				        #Password=>      '');
				        #User    =>      'funes',
				        #Password=>      '2014repositorio');
	my $smtp = Net::SMTP->new('smtp.uniandes.edu.co',25,'funes','funeseditor');

	if( !defined $smtp )
	{
		$repository->log( "Failed to create smtp connection to smtp.uniandes.edu.co");
		return( 0 );
	}


	######$smtp->auth('funes','editorfunes');
